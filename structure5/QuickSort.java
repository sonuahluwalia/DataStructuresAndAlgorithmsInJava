...
public class QuickSort
{
    ...
    public static <T extends Comparable<T>> void quickSort(Vector<T> data,int n)
    // pre: 0 <= n <= data.size()
    // post: the values in data[0..n-1] are in ascending order
    {
        quickSortRecursive(data,0,n-1);
    ...
    private static <T extends Comparable<T>> int partition(Vector<T> data, int left, int right)
    ...
    // post: data[left] placed in the correct location
    {
        while (true)
        {
            // move right "pointer" toward left
            while (left < right && 
                   data.get(left).compareTo(data.get(right)) < 0) right--;
            if (left < right) swap(data,left++,right);
            else return left;
            // move left pointer toward right
            while (left < right && 
    ...
    private static <T extends Comparable<T>> void quickSortRecursive(Vector<T> data, int left, int right)
    // pre: 0 <= left <= right < data.size()
